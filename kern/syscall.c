...
sys_env_destroy(envid_t envid)
{
	...
	// checkperm is 1 to avoid killing other tasks
...
sys_exofork(void)
{
	...
	// only user process will call this
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if (page_insert(de->env_pgdir, pp, dstva, perm)) {
		return -E_NO_MEM;
	}
	return 0;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_page_alloc:
		...
		break;
	case SYS_page_map:
		ret = sys_page_map((envid_t)a1, (void *)a2, (envid_t)a3, (void *)a4,
				(int)a5);
		break;
	case SYS_page_unmap:
